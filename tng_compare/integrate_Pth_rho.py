...
for i in range(len(mbinc_Msuns)):
    ...
    intgrnd_one = lambda r: P_one_f(r)*sigma_T/(m_e*c**2)*4.*np.pi*r**2*kpc_to_cm/(1000.)**2.
    ...
    tSZ_R = np.zeros(len(rbinc))
    for j in range(len(rbinc)):
        intgrnd_two = lambda l: 2.*P_two_f(np.sqrt(l**2. + rbinc[j]**2))*sigma_T/(m_e*c**2)*2.*np.pi*rbinc[j]*kpc_to_cm/(1000.)**2.
